...
import packageImage from "../assets/images/package3cart1.png";
...
const Package3Cart = () => {
  ...
  console.log("[Package3Cart] Component rendering");
  console.log("[Package3Cart] Location state:", location.state);
  ...
  // Default base package data - Updated for Package 3
  const defaultBasePackage = {
    title: "Swimming Pool Package",
    price: 36000,
    duration: "4-6 months",
    details: [
  ...
  useEffect(() => {
    console.log("[Package3Cart] Checking authentication");
    const checkAuth = async () => {
      const token = localStorage.getItem("authToken");
      console.log("[Package3Cart] Token found:", !!token);

      if (!token) {
        console.log(
          "[Package3Cart] No token, setting isAuthenticated to false"
        );
      ...
      try {
        ...
        const response = await axios.get(
          "future-bali-backend-production.up.railway.app/api/auth/verify-token",
          {
        ...
        console.log(
          "[Package3Cart] Token verification response:",
          response.data
        );
        ...
        console.error("[Package3Cart] Token verification failed:", error);
  ...
  // Load cart data
  useEffect(() => {
    console.log("[Package3Cart] Loading cart data");
    const loadData = () => {
      // Priority 1: Location state
      if (location.state) {
        console.log("[Package3Cart] Using location state for cart data");
        setCartData({
      ...
      console.log("[Package3Cart] LocalStorage data found:", !!saved);

      if (saved) {
        try {
          console.log("[Package3Cart] Parsing localStorage data");
          ...
          console.error("[Package3Cart] Failed to parse saved data", e);
        }
      }

      // Priority 3: Defaults
      console.log("[Package3Cart] Using default cart data");
      setCartData({
  ...
  const saveCartToDatabase = async () => {
    try {
      ...
      const response = await axios.post(
        "future-bali-backend-production.up.railway.app/api/cart/save",
        {
      ...
      console.log("[Package3Cart] Cart saved successfully:", response.data);
  ...
  const handleProceedToCheckout = async () => {
    console.log("[Package3Cart] Proceed to checkout clicked");

    if (!isAuthenticated) {
      console.log(
        "[Package3Cart] User not authenticated, redirecting to login"
      );
      navigate("/login", {
        state: {
          from: "/package3-cart",
    ...
    try {
      console.log("[Package3Cart] Saving cart to database");
      const savedCartId = await saveCartToDatabase();

      console.log("[Package3Cart] Cart saved, navigating to review-order");
      ...
      console.error("[Package3Cart] Error during checkout:", error);

      if (error.response?.status === 401) {
        console.log(
          "[Package3Cart] Authentication error, redirecting to login"
        );
        navigate("/login", {
          state: {
            from: "/package3-cart",
