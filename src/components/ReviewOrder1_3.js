...
const InheritanceForm = () => {
  ...
  // Start with only 1 contact instead of 2
  ...
  const removeContact = (contactId) => {
    // Allow removal only if more than 1 contact exists
  ...
  const handleNext = async () => {
    ...
    try {
      if (!skipForm) {
        ...
        await axios.post(
          "https://future-bali-backend-fixed-version.onrender.com/api/orders/save-inheritance",
          {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <p className="text-sm text-gray-600 mb-6 text-center">
        ...
        1 contact with phone number. You can add more contacts if needed.
      </p>
          ...
          {contacts.map((contact, index) => (
            ...
            >
              <div className="flex justify-between items-center mb-4">
                ...
                {/* Only show remove button if there's more than 1 contact */}
                {contacts.length > 1 && (
                  <button
